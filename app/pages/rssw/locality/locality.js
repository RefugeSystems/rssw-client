...
//							event.$on("modified", this.update);
			...
			for(x=0; x<this.universe.indexes.entity.listing.length; x++) {
				...
				if(buffer) {
					...
					if(this.activeLocation && this.activeLocation.auto_nearby && buffer.location === this.activeLocation.id && !buffer.hidden && !buffer.obscured && (!this.activeEntity || buffer.id !== this.activeEntity.id)) {
